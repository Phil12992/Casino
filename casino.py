...
if not st.session_state.eingeloggt:
    ...
    spiel = st.selectbox("Wähle ein Spiel:", [
        ...
        "💣 Bombenzahl",
        "🤖 Greifautomat",
        "🎰 Roulette",  # Hinzugefügtes Roulette-Spiel
        "📊 Punktestand speichern"
    ])

    # Roulette-Spiel hinzufügen
    if spiel == "🎰 Roulette":
    ...
    # Anderer Code für andere Spiele (Würfel, Münzwurf, etc.) bleibt unverändert
    elif spiel == "🎲 Würfel-Spiel":
        ...
        for zahl in st.session_state.bombenzahlen:
            if st.button(f"Zahl {zahl}"):
                if zahl == bombe:
                    ...
                    st.session_state.bombenzahlen = list(range(1, 11))  # Reset
                else:
                    punkte_update(st.session_state.punkte + 1)
                    st.success(f"👍 Zahl {zahl} war sicher. +1 Punkt!")

    elif spiel == "📊 Punktestand speichern":
        save_points(st.session_state.name, st.session_state.punkte)
        st.success("✅ Dein Punktestand wurde gespeichert!")

    # Abmelden
    if st.button("🔒 Abmelden"):
        st.session_state.eingeloggt = False
        st.session_state.clear()  # Löscht alle Session-Variablen
